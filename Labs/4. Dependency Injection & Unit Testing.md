...
1. Add a method `public Task Invoke(HttpContext context, IRequestId requestId)` and in its body log the request ID using the `ILogger` injected from the constructor and `IRequestId` injected into the `Invoke` method.
