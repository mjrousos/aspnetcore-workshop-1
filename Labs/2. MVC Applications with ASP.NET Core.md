...
1. Create a new ASP.NET Core application:
  ...
  1. Select the **Web Application (Model-View-Controller)** template
...
### Extra
1. Add a [Validation Attribute](https://docs.microsoft.com/aspnet/core/mvc/models/validation#validation-attributes) to the `Person` model to constrain the age to a range between 1 and 120. *Remember to add and run a migration.*
1. Using [Attribute Routing](https://docs.microsoft.com/aspnet/core/mvc/controllers/routing#attribute-routing), add an additional route which allows you to browse to the the `Index` action of the `PeopleController` using `http://localhost:[port]/Everyone`.
